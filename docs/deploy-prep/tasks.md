...
| T5 | Fix seed anomaly date-rollover bug (A + B), update manifest | FR-4 | ☐ | |
| T6 | Database suite green (scaled + full-scale seed tests) | FR-4 | ☐ | |
| T7 | Full Python baseline green with pipefail (116 tests, 6 packages) | FR-4/FR-6 | ☐ | |
| T8 | Web production build green | FR-6 | ☐ | |
| T9 | Playwright E2E suite green (4 tests) | FR-6 | ☐ | |
| T10 | `deploy-check.sh` + terraform fmt/validate; document machine-gap FAILs | FR-6 | ☐ | |
| T11 | Load-test smoke via curl fallback against local API | FR-6 | ☐ | |
| T12 | Bump `typescript` → 6.0.3; build gate | FR-5 | ☐ | |
| T13 | Bump `next` → 16.2.9; build + E2E gates | FR-5 | ☐ | |
| T14 | Bump `tailwindcss` → 4.3.1; build + E2E gates | FR-5 | ☐ | |
| T15 | Sync stale `README.md` status/quickstart | FR-7 | ☐ | |
| T16 | Update `AI-HANDOFF.md` + `PROJECT-MASTER-GUIDE.md` | FR-7 | ☐ | |
| T17 | Delete local `feat/*` branches verified merged; list remote for approval | FR-8 | ☐ | |
| T18 | Commit (conventional, per logical change) + push `ekam-testing` | NFR-1/NFR-3 | ☐ | |
| T19 | Final report: outcomes, skipped/high-priority, questions, gaps | FR-9 | ☐ | |
